...
from engines.pipeline import recalc_ppc, recalc_coating, recalc_hydro, recalc_dyeing, ensure_capacity_current, ensure_coating_current, ensure_cs_current, ensure_hydro_current, ensure_dyeing_current, ensure_packintqm_current, ensure_everything_current, sync_frames
from services.master_rows import apply_master_edit, rebuild_master_row
...
def make_arrow_safe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in safe.columns:
        try:
            is_numeric = pd.api.types.is_numeric_dtype(safe[col])
        except Exception:
            is_numeric = False
        if not is_numeric:
...
def render_cards(summary: dict[str, float], final_label: str = "Final Manpower"):
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card"><div class="kpi-label">{final_label}</div><div class="kpi-value">{summary['final_total']:,.0f}</div></div>
    </div>
...
def render_bottom_actions(state: dict, key_prefix: str):
    ...
    with c1:
        if st.button('Freeze Changes', key=f'{key_prefix}_freeze', width='stretch'):
            ensure_everything_current(state)
            save_snapshot(state)
            state['download_bytes'] = workbook_bytes(state)
            state['download_dirty'] = False
            st.success('Working file updated.')
    with c2:
        if st.button('Prepare Download File', key=f'{key_prefix}_prepare_download', width='stretch'):
            ensure_everything_current(state)
            state['download_bytes'] = workbook_bytes(state)
            state['download_dirty'] = False
        if state.get('download_bytes') is not None and not state.get('download_dirty', True):
            st.download_button('Download Full Excel', data=state['download_bytes'], file_name=DOWNLOAD_FILE_NAME, mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key=f'{key_prefix}_download', width='stretch')
    with c3:
...
def apply_master_filters(df: pd.DataFrame, key_prefix: str, show_section_filter: bool = True, render_widgets: bool = True) -> pd.DataFrame:
    if df.empty:
        return df
    filtered_df = df.copy()
    section_key = f'{key_prefix}_sections'
    designation_key = f'{key_prefix}_designations'
    selected_sections = st.session_state.get(section_key, [])
    selected_designations = st.session_state.get(designation_key, [])
    if render_widgets:
        col1, col2 = st.columns(2)
        if show_section_filter:
            with col1:
                section_options = filtered_df['Section'].dropna().astype(str).unique().tolist()
                selected_sections = st.multiselect('Section', section_options, default=selected_sections, key=section_key)
        with col2:
            designation_options = filtered_df['Designation'].dropna().astype(str).unique().tolist()
            selected_designations = st.multiselect('Designation', designation_options, default=selected_designations, key=designation_key)
    if show_section_filter and selected_sections:
...
def sync_ppc_changes(state: dict, edited_df: pd.DataFrame):
    ...
    if changed:
        recalc_ppc(state)
        sync_frames(state)
        state['dirty'].update({'capacity': True, 'coating': True, 'cs': True, 'hydro': True, 'dyeing': True, 'packintqm': True, 'master': True})
...
def sync_coating_changes(state: dict, edited_df: pd.DataFrame):
    ...
    if changed:
        ...
        sync_frames(state)
        state['dirty']['coating'] = False
        state['dirty']['master'] = True
        state['dirty']['master'] = True
        state['download_dirty'] = True
...
def sync_dye_input_changes(state: dict, edited_df: pd.DataFrame):
    ...
    if changed:
        recalc_dyeing(state)
        refresh_dyeing_master_rows_from_sources(state)
        state['dirty']['dyeing'] = False
        state['dirty']['master'] = True
        state['download_dirty'] = True
...
def refresh_dyeing_master_rows_from_sources(state: dict):
    for row_id in ["master.dyeing.sr_1", "master.dyeing.sr_4", "master.dyeing.sr_5"]:
        if row_id in state.get("overrides", {}):
            state["overrides"][row_id]["machine"] = False
        if row_id in state.get("master_index", {}):
...
def render_generic_section_tab(state: dict, sections: list[str], tab_key: str):
    ...
    show_df(add_total_row(compact), width='stretch', hide_index=True)
...
def render_coating_tab(state: dict):
    ...
    show_df(add_total_row(state['coating_df'].drop(columns=['row_order'])), width='stretch', hide_index=True)
    st.subheader('Coating Manpower')
    render_master_editor(state, df, 'coating_master_editor')
    show_df(add_total_row(df[['Section', 'Sr_No', 'Dept_Machine_Name', 'Designation', 'Machine_Count', 'BE_Scientific_Manpower', 'BE_Final_Manpower']]), width='stretch', hide_index=True)
...
def render_cut_and_sew_tab(state: dict):
    ...
    show_df(add_total_row(state['capacity_df'].drop(columns=['row_order'])), width='stretch', hide_index=True)
    st.subheader('CS')
    show_df(add_total_row(state['cs_df'].drop(columns=['row_order'])), width='stretch', hide_index=True)
...
def render_dyeing_tab(state: dict):
    ...
    show_df(add_total_row(state['dye_inputs_df'][['Input', 'Value']]), width='stretch', hide_index=True)
    ...
    show_df(add_total_row(ezm_out.get('final_summary_table', pd.DataFrame())), width='stretch', hide_index=True)
    left2, right2 = st.columns(2)
    with left2:
        st.caption('Drylon Selected Machines')
        show_df(add_total_row(ezm_out.get('drylon_selected_table', pd.DataFrame())), width='stretch', hide_index=True)
    with right2:
        st.caption('Cotton Selected Machines')
        show_df(add_total_row(ezm_out.get('cotton_selected_table', pd.DataFrame())), width='stretch', hide_index=True)
    ...
    with left3:
        ...
        show_df(add_total_row(jet_out.get('option1_summary', pd.DataFrame())), width='stretch', hide_index=True)
        show_df(add_total_row(jet_out.get('option1_table', pd.DataFrame())), width='stretch', hide_index=True)
    with right3:
        st.caption('Option 2 - New Machine Preference')
        show_df(add_total_row(jet_out.get('option2_summary', pd.DataFrame())), width='stretch', hide_index=True)
        show_df(add_total_row(jet_out.get('option2_table', pd.DataFrame())), width='stretch', hide_index=True)
    ...
    show_df(add_total_row(state['hydro_df'].drop(columns=['row_order'])), width='stretch', hide_index=True)
    dyeing_df = apply_master_filters(section_df(state, [DYEING_SECTION]), 'dyeing', show_section_filter=False, render_widgets=False)
...
def render_packintqm_tab(state: dict):
    ...
    if state.get('packing_tqm_source_df') is not None:
        ...
        show_df(add_total_row(src), width='stretch', hide_index=True)
    ...
    if draft_df is not None:
        show_df(add_total_row(draft_df.drop(columns=[c for c in ['row_order','row_id'] if c in draft_df.columns], errors='ignore')), width='stretch', hide_index=True)
    st.subheader('Calculated Helper Table')
    if state.get('packing_tqm_helper_df') is not None:
        show_df(add_total_row(state['packing_tqm_helper_df'].drop(columns=[c for c in ['row_order','row_id'] if c in state['packing_tqm_helper_df'].columns], errors='ignore')), width='stretch', hide_index=True)
...
def render_final_master_tab(state: dict):
    ...
    show_df(add_total_row(df.drop(columns=['row_order', 'row_id'])), width='stretch', hide_index=True)
