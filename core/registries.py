...
def dye_key(group: str, metric: str) -> str:
    return f"dyeing.{slugify(group)}.{slugify(metric)}"


def packintqm_key(line: str, metric: str) -> str:
    return f"packintqm.{slugify(line)}.{slugify(metric)}"
...
def init_nodes(parsed: dict) -> dict[str, float]:
    ...
    nodes[dye_key("ezm_paddle", "paddle_count")] = 0.0
    nodes[dye_key("ezm_paddle", "ezm_count")] = 0.0
    nodes[dye_key("jet", "machine_count")] = 0.0
    helper_df = parsed.get("packing_tqm_helper_df")
    if helper_df is not None and not helper_df.empty:
        for _, r in helper_df.iterrows():
            for metric in ["Req/Day", "Production Capacity", "Reqd. Lines", "TQM Capacity", "TQM"]:
                nodes[packintqm_key(r["Line"], metric)] = float(r[metric])
    return nodes
...
def build_row_rules(master_df) -> dict[str, RowRule]:
    ...
    set_rule("master.dyeing.sr_1", machine_source=dye_key("ezm_paddle", "paddle_count"), formula_source=dye_key("ezm_paddle", "paddle_count"), scientific_source=dye_key("ezm_paddle", "paddle_count"), formula_template="({mc}/3)*3", scientific_type="div", scientific_params={"div": 3.0, "b": 3.0})
    set_rule("master.dyeing.sr_4", machine_source=dye_key("ezm_paddle", "ezm_count"), formula_source=dye_key("ezm_paddle", "ezm_count"), scientific_source=dye_key("ezm_paddle", "ezm_count"), formula_template="({mc}/2)*3", scientific_type="div", scientific_params={"div": 2.0, "b": 3.0})
    set_rule("master.dyeing.sr_5", machine_source=dye_key("jet", "machine_count"), formula_source=dye_key("jet", "machine_count"), scientific_source=dye_key("jet", "machine_count"), formula_template="({mc}/2)*3", scientific_type="div", scientific_params={"div": 2.0, "b": 3.0})
